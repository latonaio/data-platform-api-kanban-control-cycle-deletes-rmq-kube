...
func (c *DPFMAPICaller) headerDelete(
	...
	header.KanbanControlCycle = input.Header.KanbanControlCycle
	header.IsMarkedForDeletion = input.Header.IsMarkedForDeletion
	res, err := c.rmq.SessionKeepRequest(nil, c.conf.RMQ.QueueToSQL()[0], map[string]interface{}{"message": header, "function": "KanbanControlCycleHeader", "runtime_session_id": sessionID})
	if err != nil {
